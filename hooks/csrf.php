...
class CSRF_Protection
{
  ...
  {
    if ( ! $this->CI->config->item('enable_csrf_protection'))
    {
      return;
    }
...
//    $this->CI->load->library('session');
  ...
  {  
    ...
    $exclude_methods = array("payu_response","payubiz_response","wordline_response","fetch","fetchReceipt");
    $is_method = in_array($method_name,$exclude_methods) ? TRUE : FALSE;
    // Is this a post request?
    // @link http://stackoverflow.com/questions/1372147/php-check-whether-a-request-is-get-or-post
    if ($_SERVER['REQUEST_METHOD'] == 'POST' && $isAjax==FALSE && $is_method==FALSE)
    {
